...
p = 150; q = 30;  %define the size of the image, p-y axis, q-x axis
stepsize  = 0.3;  %um
...
for cc = 1:1:7 % adjust the loop numner to make sure the effect
    if p*q-(w-1)*4 > 0
...
%% over 2490 pattern is fixed. About ten point is unsampled. Sampling these point with overlaped point
if p*q-(w-1)*4 > 0
    Nonsampledarray = zeros(p*q-(w-1)*4,1);
    t = 1;
    for i = 1:1:p*q
...
tic
[Uniformity,I] =  TrajectoryBGS_Multi_40gpu(20, 'D:\Adam\Compressive sensing axial\pattern\', 'a', Point{1:FocusNum});
toc
save('I', 'I');
