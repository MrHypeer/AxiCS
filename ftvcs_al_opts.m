...
if isfield(opts,'upsilon')
    if ~isscalar(opts.upsilon) || opts.upsilon <0
        error('opts.upsilon must be positive.');
    else if opts.upsilon > 2^13 || opts.upsilon < 2^4
            fprintf('From my experience, maybe users should choose opts.beta  between 2^4 and 2^13 as a priority.\n');
        end
    end
else
...
if isfield(opts,'upsilon0')
    if ~isscalar(opts.upsilon0) || opts.upsilon0 <= 0
        error('opts.upsilon0 is should be a positive number which is no bigger than beta.');
    end
else
    opts.upsilon0 = opts.upsilon; 
end
% initial beta
